...
int main(int argc, char *argv[])
{
    ...
    {
        cerr << "UDP3: setsockopt() failed" << endl;
        exit(-1);        
    }
    ...
    while (1)
    {
        ...
        temp += 1;
        if ( bytesRcv < 0 ) {
            cerr << "UDP3: recvfrom() failed, or connection closed" << endl;
            exit(-1);
        }

        cout << "UDP3 Client " << inet_ntoa(clientAddr.sin_addr) << ":" << clientAddr.sin_port << ": " << inBuffer << endl;
        cout << temp << endl;
        ...
        cout << "UDP3: Result of service -> " << inBuffer << endl;
