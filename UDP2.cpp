...
int main(int argc, char *argv[])
{
    ...
    {
        cerr << "UDP2: setsockopt() failed" << endl;
        exit(-1);        
    }
    ...
    while (1)
    {
        ...
        if ( bytesRcv < 0 ) {
            cerr << "UDP2: recvfrom() failed, or connection closed" << endl;
            exit(-1);
        }

        cout << "UDP2 Client " << inet_ntoa(clientAddr.sin_addr) << ":" << clientAddr.sin_port << ": " << inBuffer << endl;

        int i;
        int n = strlen(inBuffer) - 1;
        for (i=0; i < n / 2; i++)
        {
            swap(inBuffer[i], inBuffer[n - i - 1]);
        }

        cout << "UDP2: Result of service -> " << inBuffer << endl;
