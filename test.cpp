...
const int BUFFERSIZE = 5;   // Size the message buffers
...
int main(int argc, char *argv[])
{
    ...
    while (strncmp(inBuffer, "terminate", 9) != 0)
    {
        ...
        while ( (bytesRecv = recv(clientSock, inBuffer, BUFFERSIZE, 0)) > 0 ) 
        {
            cout << "received " << bytesRecv << " bytes" << endl;
            strncat(outBuffer, inBuffer, bytesRecv);
